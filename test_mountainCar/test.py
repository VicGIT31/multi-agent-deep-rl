...
def plot_rewards(show_result=False):
    ...
    plt.axhline(-110, color="green", linestyle="--", linewidth=0.8, label="seuil -110")
...
# ── 10. Training loop ─────────────────────────────────────────────────────────
num_episodes = 1000 if torch.cuda.is_available() or torch.backends.mps.is_available() else 700

# Potential-based reward shaping (entraînement uniquement, ne pollue pas l'eval) :
# Φ(s) = 100·(position + 1.2) + 1000·|vélocité| → encourage à monter ET à bouger vite.
# Le shaping ajouté est γ·Φ(s') − Φ(s) : potential-based, préserve la politique optimale
# (Ng et al. 1999). +500 au terminal pour récompenser fort l'atteinte du drapeau.
def potential(obs):
    return 100.0 * (obs[0] + 1.2) + 1000.0 * abs(obs[1])

for i_episode in range(num_episodes):
    obs_np, info = env.reset()
    state = torch.tensor(obs_np, dtype=torch.float32, device=device).unsqueeze(0)
    total_reward = 0.0

    for t in count():
        action = select_action(state)
        next_obs_np, reward, terminated, truncated, _ = env.step(action.item())
        total_reward += reward  # reward brut MountainCar — c'est ce qu'on log

        phi_curr = potential(obs_np)
        phi_next = 500.0 if terminated else potential(next_obs_np)
        shaped   = reward + GAMMA * phi_next - phi_curr
        reward_t = torch.tensor([shaped], device=device, dtype=torch.float32)
        done     = terminated or truncated

        next_state = None if terminated else \
            torch.tensor(next_obs_np, dtype=torch.float32, device=device).unsqueeze(0)
...
plt.savefig("test_mountainCar/training_curve.png", dpi=120, bbox_inches="tight")
plt.ioff()
...
SOLVE_SCORE  = -110 # seuil de réussite MountainCar-v0
N_ACTIONS    = env.action_space.n
...
ax = axes[1, 0]
action_labels = ["0 (gauche)", "1 (neutre)", "2 (droite)"]
...
fig.savefig("test_mountainCar/evaluation.png", dpi=120, bbox_inches="tight")
